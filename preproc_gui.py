...
#READ_FILENAME = "dump.txt"
READ_FILENAME = "corrected.csv"

def analyse(data_obj):
    print ("Running Pre-processor")
    start_datetime = list(map(int,data_obj.start_datetime.split(",")))
    end_datetime = list(map(int,data_obj.end_datetime.split(",")))
    ...
    offset = pd.Timestamp(datetime.datetime(*start_datetime))
    endtime = pd.Timestamp(datetime.datetime(*end_datetime))
    true_elapsed_time_seconds = (endtime-offset).total_seconds()

    RecoverAfterPowerCut.make_correction()
    ...
    return (heat_total, appl_total, save_filename)